...
func (r *DedicatedCLBListenerReconciler) reconcile(ctx context.Context, lis *networkingv1beta1.DedicatedCLBListener) error {
...
// func (r *DedicatedCLBListenerReconciler) changeState(ctx context.Context, lis *networkingv1beta1.DedicatedCLBListener, state, msg string) error {
...
func (r *DedicatedCLBListenerReconciler) cleanPodFinalizer(ctx context.Context, log logr.Logger, lis *networkingv1beta1.DedicatedCLBListener) error {
...
func (r *DedicatedCLBListenerReconciler) cleanListener(ctx context.Context, log logr.Logger, lis *networkingv1beta1.DedicatedCLBListener) error {
	changed := false
	for _, ls := range lis.Status.ListenerStatuses {
		if ls.ListenerId == "" {
			continue
		}
		// 删除监听器
	...
	log.V(7).Info("no listener need to be deleted")
	return nil
}
...
func (r *DedicatedCLBListenerReconciler) deleteCLBListener(ctx context.Context, log logr.Logger, lis *networkingv1beta1.DedicatedCLBListener) error {
	for i := range lis.Status.ListenerStatuses {
		...
		ls.ListenerId = ""
		ls.State = networkingv1beta1.DedicatedCLBListenerStateDeleted
		log.V(7).Info("listener deleted, remove listenerId from status")
		if err := r.Status().Update(ctx, lis); err != nil {
			return err
		}
...
func (r *DedicatedCLBListenerReconciler) sync(ctx context.Context, log logr.Logger, lis *networkingv1beta1.DedicatedCLBListener) error {
	if err := r.ensureListener(ctx, log, lis); err != nil {
		return err
	}
	if err := r.ensureBackend(ctx, log, lis); err != nil {
		return err
	}
	return nil
}

func getDedicatedCLBListenerPodFinalizerName(lis *networkingv1beta1.DedicatedCLBListener) string {
...
func (r *DedicatedCLBListenerReconciler) ensureBackend(ctx context.Context, log logr.Logger, lis *networkingv1beta1.DedicatedCLBListener) error {
	...
	//
	// if targetPod == nil {
	// 	if lis.Status.State == networkingv1beta1.DedicatedCLBListenerStateBound { // 但监听器状态是已占用，需要解绑
	...
	// 		}
	// 		// 更新监听器状态
	// 		if lis.Status.State != networkingv1beta1.DedicatedCLBListenerStateAvailable {
	// 			if err := r.changeState(ctx, lis, networkingv1beta1.DedicatedCLBListenerStateAvailable, ""); err != nil {
	// 				return err
	// 			}
	// 		}
	// 	}
	...
	// }
	// // 更新监听器状态
	// if lis.Status.State != networkingv1beta1.DedicatedCLBListenerStateBound {
	// 	if err := r.changeState(ctx, lis, networkingv1beta1.DedicatedCLBListenerStateBound, ""); err != nil {
	// 		return err
	// 	}
	// }
	// // 更新Pod注解
...
// func (r *DedicatedCLBListenerReconciler) syncPodDelete(ctx context.Context, log logr.Logger, lis *networkingv1beta1.DedicatedCLBListener, podFinalizerName string, pod *corev1.Pod) error {
...
//	func (r *DedicatedCLBListenerReconciler) createOneListener(ctx context.Context, log logr.Logger, lis *networkingv1beta1.DedicatedCLBListener, lbId string) (listenerId string, err error) {
...
//				)
//				err = errors.New(msg)
//				if lis.Status.State != networkingv1beta1.DedicatedCLBListenerStateFailed {
//					lis.Status.ListenerId = "" // 确保 status 里没有监听器ID，避免 ensureBackendPod 时误认为监听器已创建
//					r.changeState(ctx, lis, networkingv1beta1.DedicatedCLBListenerStateFailed, msg)
//				}
//				return
//			}
//		} else { // 没有端口冲突，创建监听器
...
func (r *DedicatedCLBListenerReconciler) ensureListener(ctx context.Context, log logr.Logger, lis *networkingv1beta1.DedicatedCLBListener) error {
	...
	for i := range lis.Status.ListenerStatuses {
		...
		case "", networkingv1beta1.DedicatedCLBListenerStatePending, networkingv1beta1.DedicatedCLBListenerStateFailed:
			...
			if existedLis != nil { // 端口冲突，如果是控制器创建的，则直接复用，如果不是，则报错引导用户人工确认手动清理冲突的监听器（避免直接重建误删用户有用的监听器）
				log = log.WithValues("listenerId", existedLis.ListenerId, "port", lis.Spec.Port, "protocol", lis.Spec.Protocol)
				log.V(5).Info("lb port already existed", "listenerName", existedLis.ListenerName)
				if existedLis.ListenerName == clb.TkePodListenerName { // 已经创建了，直接复用
					log.V(5).Info("reuse already existed listener")
					ls.ListenerId = existedLis.ListenerId
					...
					err = errors.New(msg)
					if ls.State != networkingv1beta1.DedicatedCLBListenerStateFailed {
...
// func (r *DedicatedCLBListenerReconciler) ensureOneListener(ctx context.Context, log logr.Logger, lis *networkingv1beta1.DedicatedCLBListener, ls *networkingv1beta1.ListenerStatus) error {
// 	// 如果监听器状态是 Pending，尝试创建
// 	switch ls.State {
// 	case "", networkingv1beta1.DedicatedCLBListenerStatePending, networkingv1beta1.DedicatedCLBListenerStateFailed:
...
// 		if err := r.changeState(ctx, lis, networkingv1beta1.DedicatedCLBListenerStatePending, ""); err != nil {
// 			return err
// 		}
// 		return r.createListener(ctx, log, lis)
// 	}
...
// }
//
// func (r *DedicatedCLBListenerReconciler) createListener(ctx context.Context, log logr.Logger, lis *networkingv1beta1.DedicatedCLBListener, ls *networkingv1beta1.ListenerStatus) error {
// 	lbInfos := getLbInfos(ls.CLB.ID)
...
// 	return r.changeState(ctx, lis, networkingv1beta1.DedicatedCLBListenerStateAvailable, "")
// }

// SetupWithManager sets up the controller with the Manager.
func (r *DedicatedCLBListenerReconciler) SetupWithManager(mgr ctrl.Manager, workers int) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&networkingv1beta1.DedicatedCLBListener{}).
